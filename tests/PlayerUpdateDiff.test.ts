...
describe("Player update diffing (toUpdate)", () => {
  ...
  test("goldPerMinute is packed from positive gold credited during the last 60 seconds", () => {
    const info = new PlayerInfo(
      "income",
      PlayerType.Human,
      "income_client",
    ...
    income.addGold(600n);
    game.executeNextTick();
    let packed = game.drainPackedPlayerUpdates();
    ...
    for (let i = 0; i < 600; i++) {
      game.executeNextTick();
      packed = game.drainPackedPlayerUpdates();
  ...
  test("donated gold does not count as recipient income", () => {
    ...
    const recipient = game.player("recipient_id");
    donor.addGold(1000n);
    recipient.toUpdate();
